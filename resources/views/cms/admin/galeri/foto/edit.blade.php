...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6 border-b-4 border-emerald-600 pb-2 inline-block">Edit Foto Galeri: {{ $galeriFoto->judul }}</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('cms.admin.galeri.foto.update', $galeriFoto) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="gambar" class="block text-gray-700 text-sm font-bold mb-2">Ganti Gambar Foto (Opsional):</label>
                @if ($galeriFoto->gambar_url)
                    <img src="{{ $galeriFoto->gambar_url }}" alt="Gambar Foto Saat Ini" class="w-32 h-24 object-cover rounded mb-2">
                @endif
            ...
            <div class="mb-4">
                ...
                <select name="kategori_foto_id" id="kategori_foto_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('kategori_foto_id') border-red-500 @enderror">
                    <option value="">-- Pilih Kategori --</option>
                    @foreach($kategoriFotos as $kategori)
                        <option value="{{ $kategori->id }}" {{ old('kategori_foto_id', $galeriFoto->kategori_foto_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center justify-end space-x-3">
                <a href="{{ route('cms.admin.galeri.foto.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-200">
                    Batal
                </a>
