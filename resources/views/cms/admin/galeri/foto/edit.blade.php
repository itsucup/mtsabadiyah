...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('cms.admin.galeri.foto.update', $galeriFoto) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label for="gambar" class="block text-gray-700 text-sm font-semibold mb-2">Ganti Gambar (Opsional)</label>
                ...
                @if($galeriFoto->gambar_url)
                    <div class="mt-2">
                        ...
                        <img src="{{ $galeriFoto->gambar_url }}" alt="Gambar Saat Ini" class="w-48 h-auto rounded-md object-cover mt-1">
                    </div>
                @endif
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul', $galeriFoto->judul) }}" required
            ...
            <div class="mb-6">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                          class="shadow-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $galeriFoto->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6 flex items-center">
                <input type="checkbox" id="status" name="status" value="1" {{ old('status', $galeriFoto->status) ? 'checked' : '' }}
