        ...
        <table class="min-w-full leading-normal">
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <span class="relative inline-block px-3 py-1 font-semibold leading-tight {{ $foto->status ? 'text-green-900' : 'text-red-900' }}">
                            <span aria-hidden="true" class="absolute inset-0 opacity-50 rounded-full {{ $foto->status ? 'bg-green-200' : 'bg-red-200' }}"></span>
                            <span class="relative">{{ $foto->status ? 'Aktif' : 'Draft' }}</span>
                        </span>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                        <a href="{{ route('cms.admin.galeri.foto.edit', $foto) }}" class="text-amber-600 hover:text-amber-900 mr-3">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <form action="{{ route('cms.admin.galeri.foto.destroy', $foto) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus foto ini?');">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-5 bg-white text-sm text-center text-gray-500">Tidak ada foto di galeri.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
