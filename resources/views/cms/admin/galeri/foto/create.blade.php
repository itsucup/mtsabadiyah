...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6 border-b-4 border-emerald-600 pb-2 inline-block">Tambah Foto Baru</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('cms.admin.galeri.foto.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label for="gambar" class="block text-gray-700 text-sm font-bold mb-2">Pilih Gambar:</label>
            ...
            <div class="mb-4">
                ...
                <select name="kategori_foto_id" id="kategori_foto_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('kategori_foto_id') border-red-500 @enderror">
                    ...
                    @foreach($kategoriFotos as $kategori)
                        <option value="{{ $kategori->id }}" {{ old('kategori_foto_id') == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama }}</option>
                    @endforeach
                </select>
