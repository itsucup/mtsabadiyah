                        ...
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                @if ($berita->foto_header)
                                    <img src="{{ $berita->foto_header }}" alt="{{ $berita->judul }}" class="w-16 h-12 object-cover rounded">
                                @else
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                                <span class="relative inline-block px-3 py-1 font-semibold leading-tight">
                                    <span aria-hidden class="absolute inset-0 {{ $berita->status_aktif ? 'bg-green-200' : 'bg-red-200' }} opacity-50 rounded-full"></span>
                                    <span class="relative text-{{ $berita->status_aktif ? 'green' : 'red' }}-900">{{ $berita->status_aktif ? 'Aktif' : 'Draft' }}</span>
                                </span>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center">
                                <div class="flex flex-col items-center space-y-2">
                                    <a href="{{ route('cms.berita.edit', $berita->id) }}" class="text-amber-600 hover:text-amber-900 px-3 py-1 rounded-md border border-amber-600 hover:border-amber-900 transition duration-200 w-24 text-center">
                                        <i class="fas fa-edit mr-1"></i> Edit
                                    </a>
                                    <form action="{{ route('cms.berita.destroy', $berita->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?');" class="w-24">
                                        @csrf
                                        @method('DELETE')
