...
<body class="font-inter pt-20 md:pt-24"> {{-- pt-24 (96px) untuk mobile, md:pt-28 (112px) untuk desktop --}}
    ...
    <section class="relative w-full h-[600px] overflow-hidden">
        <div id="slider" class="relative w-full h-full">
            @forelse ($headerSliders as $index => $slider)
                <img src="{{ $slider->image_url }}" class="absolute inset-0 w-full h-full object-cover {{ $index === 0 ? 'opacity-100' : 'opacity-0' }} transition-opacity duration-1000" alt="Slider Image {{ $index + 1 }}" /> {{-- Alt text sederhana --}}
            @empty
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (slides.length > 1) { // Hanya aktifkan slider jika ada lebih dari 1 gambar
                let index = 0;
                setInterval(() => {
                ...
                @forelse ($latestBeritas as $berita)
                    <div class="min-w-[300px] max-w-[300px] md:min-w-[400px] md:max-w-[400px] bg-white shadow-md rounded-lg overflow-hidden flex-shrink-0">
                        @if ($berita->header_url)
                            <img src="{{ $berita->header_url }}" class="w-full h-60 object-cover" alt="{{ $berita->judul }}" />
                        @else
            ...
            <div class="space-y-6 text-gray-800">
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p>{{ $lembagaSettings->alamat ?? 'Jl. Raya Gabus No.123, Gabus, Kab. Pati, Jawa Tengah 59173' }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p>{{ $lembagaSettings->no_telepon ?? '+62 812-3456-7890' }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-emerald-700">Fax</h3>
                        <p>{{ $lembagaSettings->no_fax }}</p>
                    </div>
                </div>
                @endif
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p>{{ $lembagaSettings->email ?? 'info@mtsabadiyah.sch.id' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Fungsi untuk meng-scroll carousel
            const scrollCarousel = (direction) => {
                const scrollAmount = carousel.querySelector('div.flex-shrink-0').offsetWidth + 16; // Lebar card + gap (space-x-4 = 16px)
                if (direction === 'left') {
            ...
            // Optional: Sembunyikan tombol jika tidak ada yang bisa di-scroll (untuk lebih kompleks)
            // Ini akan membutuhkan IntersectionObserver atau logic on scroll
            // Untuk saat ini, kita biarkan muncul di desktop.
        });
    </script>
</body>
</html>